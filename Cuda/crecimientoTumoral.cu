...
*/
	...
	__device__ __managed__ int Ayuda;
...
__global__ void funcionTransicion(int estado, int size, Celula &cell, curandState *state){
	//Variables
...
int simulacion_cancer(vector < vector< vector <Celula> > > &rejillas){
	...
	for( int dia = 1; dia < NUM_DIAS; dia++){
		...
		for (int paso = 0; paso < pasos; paso++){
			...
			funcionTransicion <<< blockCount, threadsPerBlock>>> (indice_rejilla, LONG, cell, devStates);
